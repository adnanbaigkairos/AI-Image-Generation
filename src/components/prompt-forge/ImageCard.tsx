// src/components/prompt-forge/ImageCard.tsx
'use client';
...
export default function ImageCard({ imageUrl, prompt, index }: ImageCardProps) {
  const handleDownload = () => {
    ...
    link.href = imageUrl;
    // Suggest a filename, e.g., promptforge-image-1.png
    // This might not work for all image URLs (especially data URIs from Gemini if they are not direct links)
    // For direct links or if you fetch the blob, you can set a better filename.
    // For now, let's use a generic name.
    const filename = `promptforge_image${index !== undefined ? `_${index + 1}` : ''}.png`;
